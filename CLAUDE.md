...
**開工前必讀**（依序）：
0. `notes/submission-bar-2026-09-07.md` —— **投稿門檻清單**（作者定：最高標準，寧可多花幾個月）；每個 session 對著它做
...
- **本機隊列在跑時不要改 `layerspec/`**（2026-09-09 教訓）：`trajectory_cifar.py` 的 DataLoader 子程序會重新 import 套件，改到一半的檔案讓 ResNet-50 軌跡在 epoch 57 崩潰、VGG 兩個種子沒開始，隊列還誤報完成。要重構就先 `pgrep -f trajectory_cifar` 確認沒在跑，或等隊列跑完。若非改不可：只做「加法」、寫暫存檔再 `os.replace` 原子替換、改完立刻 `python -c 'import layerspec'` 驗證（2026-09-09 加 rho_align 就是這樣做的）。
