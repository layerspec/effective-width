...
| B11 | **§VI「Where the ruler fails」一節**：池化估計量在 C=2048 過不了閘門、stem 只到上界 0.3–0.4、ConvNeXt dw/dense 分開後 p=0.21、DenseNet 池化下降 < 自身抖動、RSB 配方 τ=0.999 掉 1.7、H9.1 與 P9.3 兩個否定、SO 只實現一半、MobileNetV3 隨機初始化 R-R 0.75、κ 大時推論無資訊、白噪聲 ρ 0.32、A18／A17 若負 | 失效案例集中一節是「作者知道方法邊界」的證據 | 1 天 | ✔ 09-14：§VI-D `sec:fails`，11 個案例分三群（沉默：池化閘門、stem 資料限制＋ImageNet 只砍 stem、κ 大時推論無資訊；噪聲內：DenseNet 池化、ConvNeXt dw p=0.21、MobileNetV3 R-R 0.75、白噪聲 ρ 0.32；預測失敗：H9.1、P9.3 一半、RSB 1.7、conv 尺 C-100 −0.59＋CKA 40 通道漂移＝C3(i)、A23 空測試）。A17 未跑故不列 |
...
| B12 | **§I 拔高立意**：開場三段改為「寬度沒有尺（瓶頸）→ 三個默默的選擇讓文獻累積不起來、理論只在 lazy 體制（為何過去做不到）→ WΣWᵀ 恆等式把寬度拆成架構／資料／學習三因子，各有命題與量測（突破）」；草稿 `notes/intro-elevation-2026-09-09.md` | 資深審稿人看普適價值，不看修正一個小文獻 | 半天；**A18 與對齊結果後定稿** | ✔ 09-14 與 C1 一起定稿：§I 開場「深度有理論、寬度只有極限」→ so-what 段 → Garg／E&B → 三個默默選擇 → WΣWᵀ 支點段 |
...
| # | 調整 | 代價 | 時機 |
|---|---|---|---|
| C1 | **§I／摘要的「so what」提前**：開場改為「這把尺能指導一個設計決定（A18／A23：尺定寬重訓不掉分、同參數均勻縮輸 8/9）、能解釋一個現有方法為何失效（SO／SRIP 只實現一半增益、核 erank 0.84 未等距、對齊不變）」，量法的糾正是手段不是結論。與 B12 合併 | 半天 | ✔ 09-14：§I 第二段直接給三件事（ReLU 後尺定寬訓得到、百類以上讀出全寬＝十類效應、正交懲罰只實現一半因對齊未動）；貢獻清單加「What the ruler is for」一條；take-away 改四件事；預登記段補三個重訓實驗 |
| C2 | **A23 決定論文高度**：ImageNet (e) 不掉分 → 標題／摘要寫「在 ImageNet 從頭訓練成立」；掉分 → 照登記報、主張限定 CIFAR，機率估計回到 50% 上下，屆時再議是否補方法 | 0（判定已寫好，§24） | **09-14 收：第三種結果**——尺在 ImageNet ResNet-18 讀出全寬（99.2% 參數），(e) 不掉分但平凡（三臂差 < 種子差 0.60）；P9.29 字面成立、P9.30 不成立（plan §10.10）。摘要／§VI-C 已改寫成「CIFAR-10 的餘裕是十類效應」；**不得寫「在 ImageNet 成立」**。`layerspec shrink` 條件照字面未達成；高度與機率待作者定 |
...
## 2026-09-11 作者決定：目標是拼**一篇** TPAMI（第一篇），第二篇獨立、之後再談

- 第一篇維持量測論文；方法（訓練前的寬度判準、最大化 ReLU 後有效寬度的正則化）不併入。
- 折衷：把「量一次、照尺重訓」做成套件指令（`layerspec shrink`，任意 torchvision 分類模型的寬度改寫＋重訓＋回報準確率與 MACs），
  寫進 Reproducibility 與 §VI 作為第一篇的可用流程。**條件：A23 在 ImageNet 成立**；不成立則只保留現有 VGG／ResNet-18 腳本，不做通用改寫。
- 估計：A23 成立 60–65%、不成立 50%（major revision 以上）；十月投。
- 這個決定之後不再回頭爭論「要不要併方法」。

## 2026-09-14 作者決定：依 A23 結果調整方向

- **C2 定案**：第一篇寫「ReLU 後 τ=0.999 的閒置寬度是十類效應」（C-10 33%／C-100 1%／ImageNet 0.8%），不寫「在 ImageNet 成立」。
  機率估計回到 50% 上下（major revision 以上）。
- **`layerspec shrink` 不做**：百類以上沒有東西可縮；Reproducibility 只保留現有 VGG／ResNet-18 腳本。
- **調撥車道控制器移到第二篇**，訊號要重想（τ=0.95–0.999 斜率、或只在前段調撥），不進第一篇 §V。
  `notes/lane-idea-2026-09-11.md` 的 (a) 案作廢。
- 第一篇剩餘工作依序：B11 `sec:fails`（含 C3(i)）→ C1 §I so-what 提前（與 B12 合併）→ 全文通讀 → 十月投。

## 2026-09-14 下午：最便宜補法（A24 類別數控制）已做

plan §9.12 登記、§10.11 判定。單調性成立（3/3 種子），但「十類效應」要改成「few-class 任務的性質，隨類別數單調收斂、
且要訓練到位（影像數／步數）才出現」；兩個預測失敗照報，`sec:fails` 加第 (xii) 條。機率估計：由 50% 拉到約 55%——
受控的單調結果是加分，P9.34 反向是誠實的減分，但也把「訓練資料量」這個新變因擺上檯面。
A25（plan §9.13／§10.12，09-14 夜）：P9.35 成立，收縮是步數的性質，P9.34 的推理對、對照錯；已進論文。機率約 55–57%。
剩餘：全文通讀 → 十月投。
