...
作者再三要求題目要夠大（新理論＋新架構＋可用＋贏現有）。路 1（訓練前定寬）單獨不夠；**合併路 1＋路 2**：
「寬度不該是超參數——訓練前從資料複雜度算出每層起始寬度，訓練中依無標籤二階統計訊號在層間調撥，
同準確率下省算力」。對手與門檻見 `notes/dynamic-width-literature-2026-09-16.md`（Yuan 2023：ResNet-50 60% 訓練 FLOPs −0.8 點；
AWN ICLR 2026 佔了口號但只做 MLP）。本檔的可行性四步仍是第一段理論的起點；控制器判據推導另開檔。
