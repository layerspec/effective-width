...
the disagreement is largely about three choices made without comment, the
denominator, the covariance estimator and the tensor read, each of which
moves a profile by more than the effects reported: dividing by the
attainable rank instead of the nominal width removes an apparent
two-population structure in all 14 ImageNet models with $1\!\times\!1$
expansion layers, global pooling lowers the value on 687 of 703 dense
layers, and the block output exceeds the rank bound of its last
convolution in 161 of 161 bottleneck blocks. The resulting protocol
recovers the published CIFAR-10 profile of Garg \emph{et al.}
($r = 0.98$) and the ImageNet values of Elmoznino and Bonner
($\rho = 0.997$). Because the pre-activation output covariance is exactly
$W\Sigma_{\rm patch}W^{\top}$, the effective width separates into an
architectural, a data and a kernel factor; on 44 trained checkpoints of
eleven families measured against their own initialisations, training
moves the bottleneck from the data to the kernel, which becomes a matched
filter for its input. The widths the protocol reads after the
nonlinearity can be trained to: a VGG-16 built at them matches full width
on CIFAR-10 at two thirds of the parameters, and a pre-registered control
shows the unused width belongs to the few-class task and closes
monotonically with the number of classes. Orthogonality penalties realise
half of the width gain the decomposition predicts. Code, spectra and the
...
\num{500}\,+ ImageNet layers, report it rising with depth.
...
$W$ with $\Sigma_{\rm patch}$; each factor comes with a proposition, a
bound and a measurement. We validate the protocol against both published
...
layer (TwoNN \num{18} against \num{200} principal components at $90\%$
variance \cite{ansuini2019intrinsic}). The linear count is the object
width is about, because the consumer of a layer's output is the next
...
penultimate layer \cite{papyan2020neuralCollapse}. The bound $r_{\max}$
...
\begin{enumerate}
  ...
  \item \textbf{Three pitfalls, quantified} on 19 checkpoints: under
  $r_{\max}$ the bounded-versus-unbounded split disappears in all 14 models
  with expansion layers; the pooled estimator is lower on 687 of 703 dense
  layers, by more than the profile's own step in 18 of 19 networks; the
  block output exceeds the bound on its last convolution in 161 of 161
  bottleneck blocks (Sections~\ref{sec:rmax}--\ref{sec:blockout}).
  ...
  no block effect, so the reordering belongs to the data or the recipes;
  the ordering is in place by epoch 40--60 while accuracy is still at
  ...
  ordering follows the kernels' Gram spectra, and the kernel is a matched
  filter for its input ($\rho_{\rm align} \approx +0.9$, zero at
  initialisation). The decomposition predicts that orthogonalising a
...
$0.960$--$1.000$ (Equation~\eqref{eq:rmax} was got wrong twice during this
work; this check caught both). \emph{Bounded layers fill the bound}:
...
with expansion layers (Mann--Whitney $p < 10^{-2}$) and in none under
$r_{\max}$ ($p > 0.01$ in all 14); ResNet-50's medians move from $0.236$
against $0.938$ to $0.927$ against $0.938$, and its median
$k^*(0.95)/r_{\max}$ of $0.42$ is in line with VGG-16 ($0.42$) and
ResNet-18 ($0.53$).
...
finds no agreement in ordering ($\rho = -0.07$) while the statistics
normalised by $C$ find $0.55$--$0.58$, which is the expansion sawtooth
they share with the architecture.
...
$\rho$ median $0.41$; $-0.52$ on the ImageNet VGG-16, where PR falls from
$0.19$ to $0.02$ of $r_{\max}$ while $k^*(0.999)$ rises from $0.44$ to
$0.96$); effective rank sits between ($0.63$); RankMe orders layers as
the ruler does ($0.94$, above $0.8$ in seventeen networks of eighteen);
$\alpha$ is the most stable quantity across recipes ($0.97$) and
correlates with the ruler at $-0.66$ but is a slope, not a count. The
...
\emph{Is the block the cause?} The plan (Section~9.1) registered, before
...
recipes'. We conclude that what makes the ImageNet ResNet-50 recipes
reorder their layers is not the bottleneck block but something they share
and this experiment lacks, the dataset or the ImageNet recipes, which it
does not separate (Section~\ref{sec:limits}).
...
\emph{Exploratory; the subset and $3{,}200$ COCO images.} Varying the
protocol's choices on ResNet-50 (V1) (supplement, Table~S4), sampling 4
or 64 positions instead of 16, resizing instead of cropping, and reading
the batch-normalisation output instead of the convolution output all
leave the $k^*(0.95)/r_{\max}$ profile within one channel at every layer
($\rho \ge 0.99$). Two variations matter. On white noise the trained
profile keeps $\rho = 0.32$ of its ordering at a level of $0.24$ against
$0.42$, and $1/f$ noise recovers part of it ($0.70$), while the untrained
network moves much less ($0.78$--$0.88$): a trained profile is a property
of the network on natural images, the data factor of
Section~\ref{sec:decompose} seen from the other side. Changing the natural
images changes nothing ($3{,}200$ COCO images give the same profile to
within one channel), and the ResNet-50 bodies of COCO-trained Faster R-CNN
and Mask R-CNN order their layers like the classifier ($\rho = 0.91$) at a
lower level ($0.29$ and $0.31$ against $0.41$). The protocol reads no
...
from stage 1 to 4, the only layers below $0.7$ in all six recipes. At
$\tau = 0.95$ every role reads about one half in every stage, and by
Section~\ref{sec:projection} that is not width one can remove. The reading
...
removed when the network is retrained around the removal. Whether the
...
we do not claim the allocation is needed at this budget. The retrained
...
\emph{CIFAR-100 (Section~9.7).} With widths re-read from a CIFAR-100
...
artefact of the image count. The last layer's rank stays far above the
$C_{\rm class} - 1$ of neural collapse ($349$ directions at thirteen
classes), so what closes is a heavy tail thinning, not the subspace
theorem itself.
...
scale; the summary statistic changes the ordering, but in the direction
opposite to the one that would reconcile the shapes; the tensor and the
denominator each add a factor, the block output being higher by the
expansion ratio and the nominal denominator adding a sawtooth; and the
terminal collapse is the task's, present in our CIFAR-10 VGG-16 and
absent in our ImageNet one under one protocol, and tied by the
class-count control to few classes and long optimisation. The two
published profiles differ in what tensor they read and what task they
were trained on, and would differ in level even if they agreed on both.
...
else. The published profiles were not wrong; each is a correct
measurement of the choices it made. The paper makes two shape statements
...
(ImageNet, where the ruler found nothing to allocate) and P9.32--P9.34.
Each is reported at the place cited with its registered rule; none is
softened by an unregistered re-analysis. And $k^*(0.999)$ bounds the
...
the ImageNet effect is the dataset's or the recipes'; the experiment that
would, the same architectures under one ImageNet recipe with seeds, is
the one this study could not afford. Head depth at fixed class count was
not varied. The inverted-residual finding is about the checkpoints one
can download. A ViT-B/16 profile (supplement, Table~S6) shows the
protocol applies unchanged to linear layers; it is one checkpoint without
an initialisation control.
...
made in silence: the denominator, which misreads bottleneck networks as
underused in all 14 networks with expansion layers; the estimator, which
lowers the value at 687 of 703 layers by more than the profile's own step;
and the tensor, which differs by the expansion ratio in 161 of 161
blocks. With the choices fixed the protocol reproduces the published
CIFAR-10 table, and the identity
...
architecture gave them and gain a level set by a kernel that has become a
matched filter for its input; the ImageNet ResNet-50 checkpoints that
reorder owe it to their data or recipes, not their block; and the width
the ruler names after the nonlinearity can be trained to, the margin it
finds belonging to the few-class task. The same identity gives
orthogonality regularisation a quantified prediction, which the penalties
in use meet by half. Code, spectra and the pre-registered plan with its
deviation log are released so that the next profile can be compared with
these on stated terms.
