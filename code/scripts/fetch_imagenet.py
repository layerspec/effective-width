...
def process_shard(job) -> tuple[str, int]:
    repo, shard, out, split, cache, short_side, quality = job
    ...
    for batch in pf.iter_batches(batch_size=256, columns=["image", "label"]):
        ...
        for rec, label in zip(images, labels):
            ...
            name = name or f"{os.path.basename(shard)}_{n:07d}.JPEG"
            ...
            if not os.path.exists(dest):
                ...
                if short_side:
                    ...
                    if s < 1:
                        im = im.resize((max(short_side, round(w * s)), max(short_side, round(h * s))),
                                       Image.BILINEAR)
                else:
                    im = im.convert("RGB")
                im.save(dest + ".tmp", format="JPEG", quality=quality)
...
def main(argv=None) -> int:
    ...
    for split in splits:
        ...
        jobs = [(a.repo, s, a.out, split, cache, a.short_side, a.quality) for s in shards]
        ...
        print(f"{split}: {count} images on disk (this run wrote {total})", flush=True)
        if not a.limit_shards and count != EXPECTED[split]:
            print(f"!! expected {EXPECTED[split]} {split} images, found {count}", flush=True)
        n_cls = len([d for d in os.listdir(os.path.join(a.out, split)) if not d.startswith(".")])
        if n_cls != 1000:
            print(f"!! {n_cls} class folders in {split} (expected 1000)", flush=True)
    return 0
