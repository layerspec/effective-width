...
def section_vit(results: str, latex: str | None = None) -> None:
    ...
    if not files:
        return
    from scipy.stats import spearmanr
    hdr("21. The ruler on a transformer: ViT-B/16, trained and at initialisation (exploratory, 6,400 images x 32 tokens)")
    rows = []
    profiles = {}
    files = sorted(files, key=lambda f: f.endswith("_random_layers.csv"))  # trained first
    for f in files:
        ...
        profiles[name] = v.set_index("layer")["k_star_rmax_0.95"]
    ...
    trained = [n for n in profiles if not n.endswith("_random")]
    random_ = [n for n in profiles if n.endswith("_random")]
    if trained and random_:
        t, r = profiles[trained[0]], profiles[random_[0]]
        common = t.index.intersection(r.index)
        blocks = [l for l in common if ".blocks." in l or l.startswith("blocks.")]
        rho_all = spearmanr(t[blocks], r[blocks]).correlation
        print(f"\n  trained vs initialisation (T-R) over the {len(blocks)} block layers: Spearman {rho_all:+.2f}; "
              f"median level {t[blocks].median():.2f} trained vs {r[blocks].median():.2f} random")
        for role in ("qkv", "attention proj.", "MLP fc1", "MLP fc2"):
            lay = [l for l in blocks if _vit_role(l) == role]
            print(f"    {role:18s} T-R rho {spearmanr(t[lay], r[lay]).correlation:+.2f}; "
                  f"level {t[lay].median():.2f} -> {r[lay].median():.2f} (random)")
    if latex and rows:
        ...
        for name, role, n, C, rm, m95, lo, hi, m999, rho in rows:
            if len(profiles) > 1 and name != last:
                lab = "at initialisation (one seed)" if name.endswith("_random") else "trained (in21k$\\to$in1k)"
