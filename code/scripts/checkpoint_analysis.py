...
def section_a18(results: str, latex: str | None = None) -> None:
    ...
    for dataset, plan_sec, pnames, sub in A18_DATASETS:
        arms = {a: _a18_runs(results, a, dataset) for a in ("full", "ruler", "uniform", "ruler95")}
        if any(arms[a] for a in ("ruler", "uniform", "ruler95")):
    ...
    if latex and rows:
        ...
        dlabel = {"cifar10": "CIFAR-10 (plan 9.5)", "cifar100": "CIFAR-100 (plan 9.7)"}
        last = None
        for dataset, a, params, accs, mean, lvl in rows:
            if dataset != last:
                out.append("\\midrule")
                out.append(f"\\multicolumn{{5}}{{@{{}}l}}{{\\emph{{{dlabel[dataset]}}}}} \\\\")
            last = dataset
            out.append(f"{labels[a]} & {params / 1e6:.2f}M & {' / '.join(f'{x:.2f}' for x in accs)} & {mean:.2f} & {lvl:.2f} \\\\")
        out += ["\\bottomrule", "\\end{tabular}"]
        with open(latex, "w") as fh:
            fh.write("\n".join(out) + "\n")
        print(f"  wrote {latex}")
