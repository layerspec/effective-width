...
def _a18_runs(results: str, arm: str) -> dict:
...
def section_a18(results: str, latex: str | None = None) -> None:
    arms = {a: _a18_runs(results, a) for a in ("full", "ruler", "uniform", "ruler95")}
    if not any(arms[a] for a in ("ruler", "uniform", "ruler95")):
        return
    hdr("16. A18: widths set from the ruler, retrained from scratch (analysis-plan 9.5)")
    wpath = os.path.join(results, "a18", "widths.json")
    ...
    if "full" in means and "ruler" in means:
        ...
        p96 = (means["ruler"] - means["full"] > -0.3) and (min(r) >= means["full"] - 0.5)
        print(f"\n  P9.6 (ruler within 0.3 of full, no seed below full-0.5): diff {means['ruler'] - means['full']:+.2f}, "
              f"min ruler {min(r):.2f} vs full-0.5 {means['full'] - 0.5:.2f} -> {'SUPPORTED' if p96 else 'NOT supported'}")
    if arms["ruler"] and arms["uniform"]:
        r = [arms["ruler"][k][0] for k in arms["ruler"]]; u = [arms["uniform"][k][0] for k in arms["uniform"]]
        p97 = max(u) < min(r)
        print(f"  P9.7 (all uniform below all ruler): uniform max {max(u):.2f} vs ruler min {min(r):.2f} -> {'SUPPORTED' if p97 else 'NOT supported'}")
    if "full" in means and "ruler95" in means:
        p98 = means["full"] - means["ruler95"] >= 1.0
        print(f"  P9.8 (ruler95 loses >= 1.0 point): {means['full'] - means['ruler95']:+.2f} -> {'SUPPORTED' if p98 else 'NOT supported'}")
    if arms["ruler"]:
        lvl = float(np.median(np.concatenate([arms["ruler"][k][1] for k in arms["ruler"]])))
        print(f"  P9.9 (exploratory; ruler arm filled: median k*(.999)/r_max >= 0.9): {lvl:.3f} -> {'yes' if lvl >= 0.9 else 'no'}")
    if latex and means:
        out = ["% Generated by scripts/checkpoint_analysis.py --latex-a18.  Do not edit by hand.",
               "\\begin{tabular}{lrrrr}", "\\toprule",
               "Arm & params & acc.\\ per seed (\\%) & mean & final $k^*(0.999)/r_{\\max}$ \\\\", "\\midrule"]
        labels = {"full": "full width", "ruler": "ruler, $\\tau = 0.999$", "uniform": "uniform, same parameters", "ruler95": "ruler, $\\tau = 0.95$"}
        for a in ("full", "ruler", "uniform", "ruler95"):
            runs = arms[a]
            if not runs:
                continue
            accs = [runs[k][0] for k in sorted(runs)]
            lvl = float(np.median(np.concatenate([runs[k][1] for k in runs])))
            params = W.get("params", {}).get(a, float("nan"))
            out.append(f"{labels[a]} & {params / 1e6:.2f}M & {' / '.join(f'{x:.1f}' for x in accs)} & {np.mean(accs):.2f} & {lvl:.2f} \\\\")
        out += ["\\bottomrule", "\\end{tabular}"]
        with open(latex, "w") as fh:
            fh.write("\n".join(out) + "\n")
        print(f"  wrote {latex}")
...
def section_vit(results: str, latex: str | None = None) -> None:
    ...
    if latex and rows:
        out = ["% Generated by scripts/checkpoint_analysis.py --latex-vit.  Do not edit by hand.",
               ...
               "Role & $n$ & $C$ & $r_{\\max}$ & $k^*(0.95)/r_{\\max}$ (range) & $k^*(0.999)/r_{\\max}$ & $\\rho$(block) \\\\", "\\midrule"]
        for name, role, n, C, rm, m95, lo, hi, m999, rho in rows:
            rs = "--" if np.isnan(rho) else f"{rho:+.2f}"
            out.append(f"{role} & {n} & {C} & {rm} & {m95:.2f} ({lo:.2f}--{hi:.2f}) & {m999:.2f} & {rs} \\\\")
        out += ["\\bottomrule", "\\end{tabular}"]
        with open(latex, "w") as fh:
            fh.write("\n".join(out) + "\n")
        print(f"  wrote {latex}")
